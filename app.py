...
skills = st.sidebar.multiselect(
    "Skills", ["Python", "Data Analysis", "Machine Learning", "Web Development", "Finance", "Design"]
)
hobbies = st.sidebar.multiselect(
    "Hobbies", ["Reading", "Gaming", "Sports", "Traveling", "Music", "Art"]
)
interests = st.sidebar.multiselect(
    "Interests", ["AI", "Business", "Healthcare", "Education", "Technology", "Research"]
)
...
# ---------------- Region Selection ---------------- #
region = st.sidebar.selectbox("Preferred Region", ["India", "USA", "Europe"])
...
if "career_options" in st.session_state:
    careers = st.session_state["career_options"]["careers"]
    ...
    if selected_career and st.session_state.get("selected_career") != selected_career:
        ...
        st.session_state["conversation"] = init_chatbot(st.session_state["roadmap"], selected_career)
